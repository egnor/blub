  ...
  virtual bool maybe_receive_frame(XBeeAPI::Frame* frame) override {
    ...
    for (int i = 0; i < to_read; ++i) {
      ...
      if (state == API_MODE && frame != nullptr) {
        ...
        TL_SPAM("Incoming XBee frame (0x%02x) %d bytes", frame->type, size);
        return true;
      }
    }
    ...
    switch (state) {
      case START: {
        TL_SPAM("XBee radio setup starting");
      ...
      case INIT_9600_DELAY_PLUSPLUSPLUS: {
        if (now - state_millis > 1100) {
          TL_SPAM("XBee sending +++ at 9600, waiting for OK");
          serial->print("+++");
      ...
      case INIT_9600_EXPECT_OK: {
        if (eat_ok()) {
          TL_SPAM("XBee got OK to +++ at 9600, switching to 115200");
          ...
          TL_SPAM("XBee no OK at 9600, trying 115200");
      ...
      case INIT_SWITCH_TO_115200: {
        if (now - state_millis > 100) {
          TL_SPAM("XBee sending AT at 115200, waiting for OK");
          serial->begin(115200);
          serial->print("AT\r");
      ...
      case INIT_115200_DELAY_PLUSPLUSPLUS: {
        if (now - state_millis > 1100) {
          TL_SPAM("XBee sending +++ at 115200, waiting for OK");
          serial->print("+++");
          state = INIT_115200_EXPECT_OK;
          in_buf.clear();  // Ignore input before +++
        }
      ...
      case INIT_115200_EXPECT_OK: {
        if (eat_ok()) {
          TL_SPAM("XBee got OK at 115200, enabling API mode");
          serial->print("ATAP1,CN\r");  // enable API mode, exit command mode
          state = INIT_API_EXPECT_OKOK;
        } else if (now - state_millis > 1500) {
          TL_PROBLEM("XBee no OK at 115200, retrying init");
          serial->begin(9600);
          state = INIT_9600_DELAY_PLUSPLUSPLUS;
        }
        break;
      ...
      case INIT_API_EXPECT_OKOK: {
        if (eat_ok(2)) {  // OK for ATAP1, OK for CN
          TL_SPAM("XBee radio running in API mode");
          state = API_MODE;
        } else if (now - state_millis > 1500) {
          TL_PROBLEM("XBee no OK for API mode, retrying");
          state = INIT_115200_DELAY_PLUSPLUSPLUS;
        }
        break;
      }
      ...
      default:
        TL_FATAL("Bad XBeeRadio state: %d", state);
    }
